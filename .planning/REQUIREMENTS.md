...
- [x] **PUB-03**: The owner approved repository publication of the Daily Market Dashboard on 2026-08-18. It derives deterministic exact-product daily medians, retained low/high evidence and same-product adjacent-day direction from the approved factual-offer payload. Ranges longer than 30 days plot one rounded arithmetic average of daily typical prices per observed month; absent months remain gaps. Production deployment remains separately digest-locked.
- [x] **PUB-04**: The owner approved the empty public Event Line surface on 2026-08-18. Any real marker still requires a separately approved exact revision with citation, author state, event/publication dates, uncertainty and counter-evidence. Marker changes cannot affect numeric output, and private review identifiers must not enter the public payload.
- [x] **PUB-05**: The owner approved the eight-retailer exact-MPN comparison roster and comparison layout on 2026-08-24. The RAM workspace renders a deterministic matrix of released exact MPNs by approved retailer, uses an explicit unavailable-evidence blank for absent pairs, and retains exactly three lower/middle/higher spotlights selected from one latest qualifying observation per released product. A roster entry grants no automatic-fetch, methodology, ranking, recommendation or completeness authority.
