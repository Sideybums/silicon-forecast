...
const OVERLAY_FIELDS = [
  ...
  "display_order", "creator_id", "review_state", "reviewer_id",
];
...
function withoutClaimHash(claim) {
  const copy = structuredClone(claim);
  delete copy.claim_revision_hash;
  return copy;
}
...
function unique(values, context) {
  ...
  for (const value of values) {
    invariant(!seen.has(value), `${context} must be unique`);
    seen.add(value);
  }
}
...
function validateOverlayRevision(revision, knownEvents, knownNumeric, context) {
  ...
  invariant(revision.reviewer_id === null || (typeof revision.reviewer_id === "string" && ID.test(revision.reviewer_id)), `${context}.reviewer_id must be null or an attributable stable ID`);
  invariant((revision.review_state === "reviewed_private") === (revision.reviewer_id !== null), `${context} reviewed_private requires exactly one attributable reviewer`);
  invariant(revision.reviewer_id === null || revision.reviewer_id !== revision.creator_id, `${context} creator cannot review their own overlay revision`);
...
function validateFixture(fixture) {
  ...
  for (const [index, capture] of fixture.captures.entries()) {
    ...
    exactKeys(capture, ["capture_id", "source_item_id", "retrieved_at", "status", "mime_type", "byte_count", "sha256", "retained_representation", "retained_bytes_utf8", "prompt_injection_indicators", "active_content_executed"], context);
    id(capture.capture_id, `${context}.capture_id`);
    invariant(!captureIds.has(capture.capture_id), `${context}.capture_id must be unique`);
    ...
    invariant(typeof capture.retained_bytes_utf8 === "string", `${context}.retained_bytes_utf8 must retain exact inert bytes`);
    invariant(Buffer.byteLength(capture.retained_bytes_utf8, "utf8") === capture.byte_count, `${context}.byte_count disagrees with retained_bytes_utf8`);
    invariant(digest(capture.retained_bytes_utf8) === capture.sha256, `${context}.sha256 disagrees with retained_bytes_utf8`);
  ...
  for (const [index, claim] of fixture.atomic_claims.entries()) {
    ...
    exactKeys(claim, ["claim_id", "claim_revision_hash", "capture_id", "capture_sha256", "exact_extract_utf8", "claim_text", "claim_type", "pinpoint_locator", "valid_time", "status", "duplicate_lineage_id"], context);
    id(claim.claim_id, `${context}.claim_id`);
    invariant(!claimIds.has(claim.claim_id), `${context}.claim_id must be unique`);
    sha(claim.claim_revision_hash, `${context}.claim_revision_hash`);
    invariant(claim.claim_revision_hash === digest(withoutClaimHash(claim)), `${context}.claim_revision_hash does not bind exact immutable claim content`);
    invariant(captureIds.has(claim.capture_id), `${context} references unknown capture`);
    const capture = fixture.captures.find((item) => item.capture_id === claim.capture_id);
    invariant(claim.capture_sha256 === capture.sha256, `${context}.capture_sha256 must bind the exact referenced capture bytes`);
    invariant(typeof claim.exact_extract_utf8 === "string" && claim.exact_extract_utf8.length > 0, `${context}.exact_extract_utf8 is required`);
    invariant(capture.retained_bytes_utf8.includes(claim.exact_extract_utf8), `${context}.exact_extract_utf8 must occur byte-for-byte in the retained capture`);
  ...
  for (const [index, relation] of fixture.evidence_relationships.entries()) {
    ...
    invariant(!relationshipIds.has(relation.relationship_id), `${context}.relationship_id must be unique`);
  ...
  for (const [index, event] of fixture.event_revisions.entries()) {
    ...
    exactKeys(event, ["event_id", "revision", "revision_hash", "supersedes_revision_hash", "title", "event_date", "date_precision", "factual_state", "editorial_state", "claim_revision_refs", "contradictory_claim_revision_refs", "uncertainties", "alternative_explanations", "causal_language_level", "author_id"], context);
    ...
    invariant(!eventHashes.has(event.revision_hash), `${context}.revision_hash must be unique`);
    eventHashes.add(event.revision_hash);
    invariant(event.supersedes_revision_hash === null || SHA256.test(event.supersedes_revision_hash), `${context}.supersedes_revision_hash must be null or SHA-256`);
    ...
    for (const field of ["claim_revision_refs", "contradictory_claim_revision_refs"]) {
      invariant(Array.isArray(event[field]), `${context}.${field} must be an array`);
      unique(event[field].map((ref) => `${ref.claim_id}\u0000${ref.claim_revision_hash}`), `${context}.${field}`);
      for (const [refIndex, ref] of event[field].entries()) {
        exactKeys(ref, ["claim_id", "claim_revision_hash"], `${context}.${field}[${refIndex}]`);
        const claim = claimsById.get(ref.claim_id);
        invariant(claim && claim.claim_revision_hash === ref.claim_revision_hash, `${context}.${field}[${refIndex}] must bind an exact immutable claim revision`);
      }
    }
    invariant(event.factual_state !== "contested" || event.contradictory_claim_revision_refs.length > 0, `${context} contested event must expose contradiction`);
    ...
    invariant(!eventMap.has(eventKey), `${context} event revision identity must be unique`);
    const prior = eventLatest.get(event.event_id);
    invariant(prior ? event.revision === prior.revision + 1 && event.supersedes_revision_hash === prior.revision_hash : event.revision === 1 && event.supersedes_revision_hash === null, `event ${event.event_id} must be an additive revision chain`);
    eventMap.set(eventKey, event);
    eventLatest.set(event.event_id, event);
  }
  ...
  for (const [index, review] of fixture.reviews.entries()) {
    ...
    invariant(!reviewIds.has(review.review_id), `${context}.review_id must be unique`);
    ...
    const reviewedEvent = [...eventMap.values()].find((event) => event.event_id === review.record_id && event.revision_hash === review.revision_hash);
    invariant(reviewedEvent, `${context} must bind an exact immutable event revision`);
    ...
    invariant(review.reviewer_id !== reviewedEvent.author_id, `${context} event author cannot review their own revision`);
  ...
  const overlayIdentities = new Set();
  for (const [index, overlay] of fixture.overlay_revisions.entries()) {
    ...
    const identity = `${overlay.overlay_id}\u0000${overlay.revision}`;
    invariant(!overlayIdentities.has(identity), `overlay_revisions[${index}] identity must be unique`);
    overlayIdentities.add(identity);
...
export function createCandidateEventOverlayModel(fixture) {
  ...
  const model = {
    ...
    presentation_order: [...overlayLatest.values()].filter((item) => item.placement_state === "placed_private" && item.review_state === "reviewed_private").sort((a, b) => a.display_order - b.display_order || a.overlay_id.localeCompare(b.overlay_id)).map((item) => item.overlay_id),
  };
...
function appendRevision(state, candidate, internals) {
  ...
  invariant(!state.overlay_revisions.some((revision) => revision.revision_hash === candidate.revision_hash), "operation overlay revision hash must be unique");
  invariant(!state.overlay_revisions.some((revision) => revision.overlay_id === candidate.overlay_id && revision.revision === candidate.revision), "operation overlay revision identity must be unique");
...
export function applyOverlayOperation(model, operation) {
  ...
  exactKeys(operation, ["action", "actor_id", "actor_type", "revision"], "overlay operation");
  ...
  id(operation.actor_id, "overlay operation.actor_id");
  ...
  if (operation.action === "add") {
    invariant(!latest, "add cannot overwrite an existing overlay");
    invariant(operation.revision.review_state !== "reviewed_private" && operation.revision.reviewer_id === null, "add cannot submit reviewed_private or bypass review");
    invariant(operation.revision.creator_id === operation.actor_id, "add creator must be the attributable actor");
  ...
  if (operation.action === "remove") {
    ...
    assertOnlyRevisionFieldsChanged(latest, operation.revision, ["placement_state", "review_state", "reviewer_id"], "remove");
  }
  if (operation.action === "reorder") {
    ...
    invariant(operation.revision.review_state === "review_pending" && operation.revision.reviewer_id === null, "reordered overlay must return to attributable review_pending state");
    assertOnlyRevisionFieldsChanged(latest, operation.revision, ["display_order", "review_state", "reviewer_id"], "reorder");
  }
  if (operation.action === "review") {
    ...
    invariant(operation.revision.placement_state === "placed_private", "review cannot approve an unavailable or withdrawn overlay");
    ...
    invariant(operation.actor_id !== latest.creator_id, "overlay creator cannot review their own work");
    invariant(operation.revision.reviewer_id === operation.actor_id, "reviewer_id must identify the attributable reviewing actor");
    assertOnlyRevisionFieldsChanged(latest, operation.revision, ["review_state", "reviewer_id"], "review");
  }
  appendRevision(state, operation.revision, internals);
  const current = [...latestOverlays(state.overlay_revisions).values()];
  state.presentation_order = current.filter((item) => item.placement_state === "placed_private" && item.review_state === "reviewed_private").sort((a, b) => a.display_order - b.display_order || a.overlay_id.localeCompare(b.overlay_id)).map((item) => item.overlay_id);
