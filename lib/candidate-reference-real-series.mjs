...
function aggregateMonths(fixture, strategy) {
  ...
  invariant(Array.isArray(strategy.month_contracts) && strategy.month_contracts.length > 0, "monthly contracts are required");
  const seenMonths = new Set();
  for (const contract of strategy.month_contracts) {
    ...
    invariant(!seenMonths.has(contract.month), `duplicate monthly contract ${contract.month}`);
    seenMonths.add(contract.month);
    const seenRequiredDates = new Set();
    for (const date of contract.required_dates) {
      invariant(DATE.test(date), `monthly contract ${contract.month} required date is invalid`);
      invariant(date.startsWith(`${contract.month}-`), `monthly contract ${contract.month} contains cross-month required date ${date}`);
      invariant(!seenRequiredDates.has(date), `monthly contract ${contract.month} contains duplicate required date ${date}`);
      seenRequiredDates.add(date);
    }
  }
...
function derivedSeriesRevisions(approvalEnvelope, outputs) {
  const inputBindingSha256 = sha256FixtureBytes(approvalEnvelope.binding);
  return {
    monthly_nominal: {
      series_id: "synthetic-fixture-monthly-linked-nominal",
      revision_id: approvalEnvelope.envelope_sha256,
      input_binding_sha256: inputBindingSha256,
      output_sha256: sha256FixtureBytes(outputs.monthly_nominal),
    },
    historical_reference_100: {
      series_id: "synthetic-fixture-historical-reference-100",
      revision_id: approvalEnvelope.envelope_sha256,
      input_binding_sha256: inputBindingSha256,
      output_sha256: sha256FixtureBytes(outputs.historical_reference_100),
    },
    monthly_constant_price: {
      series_id: "synthetic-fixture-monthly-constant-price",
      revision_id: approvalEnvelope.envelope_sha256,
      input_binding_sha256: inputBindingSha256,
      output_sha256: sha256FixtureBytes(outputs.monthly_constant_price),
    },
  };
}
...
export function deriveCandidateReferenceRealSeries(fixture, approvalEnvelope = null) {
  ...
  return {
    ...
    series_revisions: derivedSeriesRevisions(approvalEnvelope, {
