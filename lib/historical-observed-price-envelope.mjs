...
// Plain codepoint comparison. Array.prototype.sort's default comparator
// already does this, but we spell it out because it must never be swapped
// back for String#localeCompare: ICU collation is case- and
// hyphen-insensitive at the primary comparison level, so it can disagree
// with codepoint order for these id shapes, is not guaranteed stable across
// ICU builds or LC_ALL, and can silently change tie-break outcomes (and
// therefore golden fixture bytes) between environments.
...
function normaliseVatState(value) {
  ...
  // Fail closed. VAT-unresolved observations are now INCLUDED in the
  // envelope under an approved relaxation, so a silently-accepted typo
  // (e.g. "exlcuded") would admit an ex-VAT price into the band as if it
  // were merely unresolved. Family A's real observation files (checked
  // across all four eligible tranches) only ever use the literal string
  // "included" today — no genuine unresolved/unknown marker has been
  // observed — so there is nothing to whitelist beyond the two known-good
  // values; anything else, including a missing field, is a data error.
  throw new Error(`item_price.vat_state must be "included" or "excluded", got: ${JSON.stringify(value)}`);
}

const REQUIRED_CAPACITY_GB = 32;
const REQUIRED_MODULE_COUNT = 2;
const REQUIRED_MEMORY_TYPE = "DDR5";
...
export function normaliseObservation(raw, { sourceFile, captureKind }) {
  ...
  if (raw.price && typeof raw.price === "object") {
    ...
    // This schema records physical-kit facts directly on the observation, so
    // we can and must reject anything that is not the 32GB/2-module/DDR5
    // shape the envelope is scoped to (a 16GB kit, a 64GB kit or a DDR4 part
    // would otherwise enter the band silently).
    invariant(
      raw.product?.capacity_gb === REQUIRED_CAPACITY_GB,
      `product.capacity_gb must be ${REQUIRED_CAPACITY_GB}, got: ${raw.product?.capacity_gb} (${raw.observation_id})`,
    );
    invariant(
      raw.product?.module_count === REQUIRED_MODULE_COUNT,
      `product.module_count must be ${REQUIRED_MODULE_COUNT}, got: ${raw.product?.module_count} (${raw.observation_id})`,
    );
    invariant(
      raw.product?.memory_type === REQUIRED_MEMORY_TYPE,
      `product.memory_type must be ${REQUIRED_MEMORY_TYPE}, got: ${raw.product?.memory_type} (${raw.observation_id})`,
    );
    invariant(
      raw.eligibility?.identity_exact === true,
      `eligibility.identity_exact must be true, got: ${raw.eligibility?.identity_exact} (${raw.observation_id})`,
    );
  } else if (raw.item_price && typeof raw.item_price === "object") {
    // Family A — v1 retail shape. Unlike Family B, this schema does not
    // record capacity_gb, module_count or memory_type anywhere on the
    // observation itself (verified against every observation in all four
    // Family A tranche files currently in ELIGIBLE_TRANCHES) — those facts
    // are established upstream, by the catalogue-driven curation that
    // selects which product pages get captured in the first place, not by
    // anything present in this raw record. We deliberately do not invent a
    // default (e.g. assuming every Family A row is 32GB/2-module/DDR5)
    // because that would be exactly the kind of silent, unverified
    // assumption this validation exists to prevent. This is a known,
    // documented gap in per-record verification for Family A, not an
    // oversight: a future Family A tranche carrying an ineligible part
...
export function deriveObservedPriceEnvelope(observations) {
  ...
  const periods = quarterRange(present[0], present.at(-1)).map((periodId) => {
    ...
    const records = (byQuarter.get(periodId) ?? []).slice().sort((a, b) => compareCodepoint(a.observation_id, b.observation_id));
...
export const ELIGIBLE_TRANCHES = [
  ...
  { file: "uk-primary-retail-historical-backfill-2026-08-10T130500Z.v1.json", captureKind: "archive_capture" },
  { file: "uk-primary-retail-scan-depth-2026-08-10T173000Z.v1.json", captureKind: "archive_capture" },
];
