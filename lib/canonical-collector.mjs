 ...
 */
export async function collectTarget(target, { fetchImpl = fetch, robotsFor = async () => "" } = {}) {
  const base = {
  ...
  // Identity must match what the registry asked for.
...
export async function runCollection(targets, options = {}) {
  const { fetchImpl = fetch, delayMs = 2500, onProgress = () => {} } = options;
  ...
  for (const [i, target] of targets.entries()) {
    const result = await collectTarget(target, { fetchImpl, robotsFor });
    results.push(result);
