...
function validateObservation(observation, trancheId) {
  ...
  return {
    ...
    evidenceExtractSha256: observation.evidence.extract_sha256,
...
function observationRows(history) {
  return history.lines.flatMap((line) => line.points.map((point) => `<tr><td>${escapeHtml(line.mpn)}</td><td>${escapeHtml(point.sellerLegalName)}</td><td><code>${escapeHtml(point.lineKey)}</code></td><td><time datetime="${escapeHtml(point.observedAt)}">${escapeHtml(point.observedAt)}</time></td><td>${pounds(point.amountMinor)}</td><td>${point.provenance}</td><td><code>${escapeHtml(point.observationId)}</code></td><td><code>${escapeHtml(point.trancheId)}</code></td><td><code>${escapeHtml(point.sourceKey)}</code></td><td><code>${escapeHtml(point.evidenceExtractSha256)}</code></td><td>Abstain: ${escapeHtml(point.reasons.join(", "))}</td></tr>`)).join("");
}
