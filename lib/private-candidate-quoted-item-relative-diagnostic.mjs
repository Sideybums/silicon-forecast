...
function lineKey(identity) {
  return `${identity.retailer_legal_name}\u0000${identity.mpn}`;
}
...
function validateTranches(tranches) {
  ...
  for (const [trancheIndex, tranche] of tranches.entries()) {
    ...
    for (const [observationIndex, observation] of tranche.observations.entries()) {
      ...
      invariant(!lineDates.has(duplicateKey), `duplicate observation for seller legal entity ${identity.retailer_legal_name}, MPN ${identity.mpn} on ${date} requires an approved same-line/same-date selection rule`);
