...
import { loadJson, normaliseObservation } from "./historical-observed-price-envelope.mjs";
import { includedCandidateInputs } from "./candidate-input-manifest.mjs";

export const SERIES_VERSION = 1;

export const seriesTranches = (root) => includedCandidateInputs(root);
...
export function loadSeriesObservations(root) {
  const records = [];
  for (const tranche of seriesTranches(root)) {
