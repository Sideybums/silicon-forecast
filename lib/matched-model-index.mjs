...
export function deriveMatchedModelIndex(series, parameters = INDEX_PARAMETERS) {
  ...
  const ordered = [...periods.keys()].sort(compareCodepoint);
  if (!periods.has(base)) {
  ...
  const describe = (period, value, link, matchedCount, pairs) => {
    const relatives = pairs.map((p) => p.to / p.from).sort((x, y) => x - y);
    return {
      period_id: period,
      state: value === null ? "insufficient_matched_evidence" : "observed",
      ...
      distinct_products_in_period: periods.get(period).size,
    };
  };
  ...
  for (let i = baseIndex + 1; i < ordered.length; i += 1) {
    const { matched_count, link, pairs } = linkFor(ordered[i - 1], ordered[i]);
    if (link === null) {
      rows.set(ordered[i], describe(ordered[i], null, null, matched_count, pairs));
      break;
    }
    value *= link;
    rows.set(ordered[i], describe(ordered[i], value, link, matched_count, pairs));
  }
  ...
  for (let i = baseIndex; i > 0; i -= 1) {
    const { matched_count, link, pairs } = linkFor(ordered[i - 1], ordered[i]);
    if (link === null) {
      rows.set(ordered[i - 1], describe(ordered[i - 1], null, null, matched_count, pairs));
      break;
    }
    value /= link;
    // The link belongs to the later period; the earlier row carries the level.
    rows.set(ordered[i - 1], { ...describe(ordered[i - 1], value, null, matched_count, pairs), link: null });
  }
  ...
  return {
    ...
    coverage: {
      ...
      periods_with_evidence_outside_chain: ordered.length - observed.length,
      total_periods_with_observations: ordered.length,
    },
    periods: chained,
  };
}
